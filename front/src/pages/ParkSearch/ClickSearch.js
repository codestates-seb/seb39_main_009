...
const ClickSearch = (props) => {
  ...
  const truePartnership = props.lala.data.filter(
    (el) => el.partnership === true
  );
  const falsePartnership = props.lala.data.filter(
    (el) => el.partnership === false
  );
  ...
  return (
    <div>
      ...
      <select>
        <option value="인기순" selected="selected">
          인기순
        </option>
      </select>
